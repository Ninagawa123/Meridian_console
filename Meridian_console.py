...
class MeridianConsole:
    def __init__(self):
        ...
        self.s_meridim_js_sub = [0]*MSG_SIZE        # ROSからサブスクライブしたサーボ位置情報の格納用Meridim配列
        self.s_meridim_motion = [0]*MSG_SIZE        # Meridim配列のPC側で作成したサーボ位置命令送信用
    ...
    while (True):
        ...
        with closing(sock):
            while True:
                ...
                mrd.r_meridim_ushort = struct.unpack('90H', _r_bin_data)  # unsignedshort型
                ...
                # エラーフラグ各種のカウントアップ
                _temp_int16 = np.array([0], dtype=np.int16) # short型のビットをpythonで扱うためのnumpy配列テンポラリ変数
...
#                    mrd.flag_send_data_step_frame == False
                ...
                ######## [3-1] 送信用UDPデータの作成 ############################################                        
                    ...
                    if mrd.flag_ros1_sub:
                        for i in range(15):
                            mrd.s_meridim_motion[21+i * 2] = mrd.s_meridim_js_sub[21+i*2]
                            mrd.s_meridim_motion[51+i * 2] = mrd.s_meridim_js_sub[51+i*2]
                    ...
                    if mrd.flag_demo_action:
                        ...
                        mrd.s_meridim_motion[21] = int(np.sin(mrd.x)*3000)           # 頭ヨー
                        mrd.s_meridim_motion[23] = int(np.sin(mrd.x)*1000) + 2000    # 左肩ピッチ
                        mrd.s_meridim_motion[25] = -int(np.sin(mrd.x*2)*1000) + 1000 # 左肩ロール
                        mrd.s_meridim_motion[27] = int(np.sin(mrd.x)*1000) + 1000    # 左肘ヨー
                        mrd.s_meridim_motion[29] = int(np.sin(mrd.x)*3000) - 3000    # 左肘ピッチ
                        mrd.s_meridim_motion[31] = int(np.sin(mrd.x)*500)            # 左股ヨー
                        mrd.s_meridim_motion[33] = -int(np.sin(mrd.x)*400)           # 左股ロール
                        mrd.s_meridim_motion[35] = int(np.sin(mrd.x*2)*2000) - 200   # 左股ピッチ
                        mrd.s_meridim_motion[37] = -int(np.sin(mrd.x*2)*4000)        # 左膝ピッチ
                        mrd.s_meridim_motion[39] = int(np.sin(mrd.x*2)*2000) - 200   # 左足首ピッチ
                        mrd.s_meridim_motion[41] = int(np.sin(mrd.x)*400)            # 左足首ロール
                        mrd.s_meridim_motion[51] = -int(np.sin(mrd.x)*2000)          # 腰ヨー
                        mrd.s_meridim_motion[53] = -int(np.sin(mrd.x)*1000) + 2000   # 右肩ピッチ
                        mrd.s_meridim_motion[55] = -int(np.sin(mrd.x*2)*1000) + 1000 # 右肩ロール
                        mrd.s_meridim_motion[57] = -int(np.sin(mrd.x)*1000) + 1000   # 右肘ヨー
                        mrd.s_meridim_motion[59] = -int(np.sin(mrd.x)*3000) - 3000   # 右肘ピッチ
                        mrd.s_meridim_motion[61] = -int(np.sin(mrd.x)*500)           # 右股ヨー
                        mrd.s_meridim_motion[63] = int(np.sin(mrd.x)*400)            # 右股ロール
                        mrd.s_meridim_motion[65] = -int(np.sin(mrd.x*2)*2000) - 200  # 右股ピッチ
                        mrd.s_meridim_motion[67] = int(np.sin(mrd.x*2)*4000)         # 右膝ピッチ
                        mrd.s_meridim_motion[69] = -int(np.sin(mrd.x*2)*2000) - 200  # 右足首ピッチ
                        mrd.s_meridim_motion[71] = -int(np.sin(mrd.x)*400)           # 右足首ロール
...
#                        dpg.set_value("transaction_mode", "Step")
                        ...
                        for i in range(8):
                            if ((mrd.s_minitermnal_keep[i][0] >= 0) and (mrd.s_minitermnal_keep[i][0] < MSG_SIZE)):
                                ...
                                print_string = print_string + \
                                    "["+str(int(mrd.s_minitermnal_keep[i][0]))+"] " + str(int(mrd.s_minitermnal_keep[i][1]))+", "
                ...
                ######## [5-1] 表示処理 ##########################################################################################
                    mrd.message2 = "ERROR COUNT ESP-PC:"+str("{:}".format(mrd.error_count_esp_to_pc)) + " PC-ESP:"+str("{:}".format(mrd.error_count_pc_to_esp))+" ESP-TSY:"+str("{:}".format(mrd.error_count_esp_to_tsy)) + " TSY_Delay:"+str("{:}".format(mrd.error_count_tsy_delay)) + "    Servo_trouble:"+mrd.error_servo_id

                    mrd.message3 = "ERROR RATE ESP-PC:"+str("{:.2%}".format(mrd.error_count_esp_to_pc/mrd.loop_count)) + " PC-ESP:"+str("{:.2%}".format(mrd.error_count_pc_to_esp/mrd.loop_count))+" ESP-TSY:"+str("{:.2%}".format(mrd.error_count_esp_to_tsy/mrd.loop_count)) + " TsySKIP:"+str("{:.2%}".format(mrd.error_count_tsy_skip/mrd.loop_count)) + " ESPSKIP:" + str("{:.2%}".format(mrd.error_count_esp_skip/mrd.loop_count))

                    mrd.message4 = "SKIP COUNT Tsy:" + str("{:}".format(mrd.error_count_tsy_skip))+" ESP:"+str("{:}".format(mrd.error_count_esp_skip))+" PC:"+str("{:}".format(mrd.error_count_pc_skip)) + " Servo:"+str("{:}".format(mrd.error_count_servo_skip))+" PCframe:"+str(mrd.loop_count)+" BOARDframe:"+str(mrd.frame_sync_r_resv)+" "+str(int(mrd.loop_count/now))+"Hz"

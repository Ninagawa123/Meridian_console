...
# ROSの環境で使用する場合は、63行目あたりのimport rospyとfrom sensor_msgs.msg import JointState
# をアンコメントアウトしてください
#
# ・各ウィンドウについて
# 【Axis Monitor】
...
class MeridianConsole:
    def __init__(self):
        ...
        self.r_meridim_char = np.zeros(MSG_SIZE*2, dtype=np.int8)  # Meridim配列
        # self.r_meridim_char = list(range(MSG_SIZE*2))  # Meridim配列の受信値char表示用
        # self.r_meridim = [0]*MSG_SIZE             # Meridim配列の送信値用
        # self.s_meridim = [0]*MSG_SIZE             # Meridim配列の送信値用
        ...
        self.s_minitermnal_keep = np.empty((8, 2))  # コンパネからのリモコン入力用
...
# 制御コマンド用フラグ等
        ...
        self.flag_resv_data = 0      # ESP32からの状態データの受信のオンオフフラグ（モーション送信時のシミュレーション空間用として）
        ...
        self.flag_set_flow = 0       # Meridanの循環を通常モードに
        self.flag_set_step = 0
    ...
    while (True):
        ...
        with closing(sock):
            while True:
                ...
                _r_bin_data, addr = sock.recvfrom(
                    MSG_BUFF)     # UDPに受信したデータを転記

                while _r_bin_data_past == _r_bin_data:  # ここで受信データの更新を待つ
                    _r_bin_data, addr = sock.recvfrom(
                        MSG_BUFF)  # UDPに受信したデータを転記しキープ

                # short型のMeridim90を作成
                mrd.r_meridim = struct.unpack('90h', _r_bin_data)
                _r_meridim_ushort = struct.unpack(
                    '90H', _r_bin_data)  # unsignedshort型のMeridim90を作成
                # 読み取り用のchar型Meridim(180)を作成
                ...
                mrd.frame_sync_r_resv = _r_meridim_ushort[1]  # 受信カウントを代入
                ...
                if (_checksum[0] == mrd.r_meridim[MSG_SIZE-1]) and (mrd.frame_sync_r_resv != mrd.frame_sync_r_last):
                    ...
                    if mrd.flag_demo_action:
                        ...
                        mrd.s_meridim_motion[71] = - \
                            int(np.sin(mrd.x)*400)          # 右足首ロール
                    ...
                    elif mrd.flag_set_flow == 1:  # フローモードへの切り替え
                        mrd.s_meridim[0] = 5  # 5 はフローモード
                        mrd.flag_set_flow = 0

                    elif mrd.flag_set_step == 1:  # ステップモードへの切り替え
                        mrd.s_meridim[0] = 6  # 6 はステップモード
                        mrd.flag_set_step = 0
                    else:
                    ...
                    _checksum_int = 0
                    for i in range(MSG_SIZE-1):
                        _checksum_int += mrd.s_meridim[i]
                    _checksum[0] = ~_checksum_int
                    ...
                    mrd.message2 = "ERROR COUNT ESP-PC:"+str("{:}".format(mrd.error_count_esp_to_pc)) +\
                        " PC-ESP:"+str("{:}".format(mrd.error_count_pc_to_esp))+" ESP-TSY:"+str("{:}".format(mrd.error_count_esp_to_tsy)) + " TSY_Delay:"+str("{:}".format(mrd.error_count_tsy_delay)) +\
                        "    Servo_trouble:"+mrd.error_servo_id

                    mrd.message3 = "ERROR RATE ESP-PC:"+str("{:.2%}".format(mrd.error_count_esp_to_pc/mrd.loop_count)) +\
                        " PC-ESP:"+str("{:.2%}".format(mrd.error_count_pc_to_esp/mrd.loop_count))+" ESP-TSY:"+str("{:.2%}".format(mrd.error_count_esp_to_tsy/mrd.loop_count)) +\
                        " TsySKIP:"+str("{:.2%}".format(mrd.error_count_tsy_skip/mrd.loop_count)) + \
                        " ESPSKIP:" + \
                        str("{:.2%}".format(
                            mrd.error_count_esp_skip/mrd.loop_count))

                    mrd.message4 = "SKIP COUNT Tsy:" +\
                        str("{:}".format(mrd.error_count_tsy_skip))+" ESP:"+str("{:}".format(mrd.error_count_esp_skip))+" PC:"+str("{:}".format(mrd.error_count_pc_skip)) +\
                        " Servo:"+str("{:}".format(mrd.error_count_servo_skip))+" PCframe:"+str(
                            mrd.loop_count)+" BOARDframe:"+str(mrd.frame_sync_r_resv)+" "+str(int(mrd.loop_count/now))+"Hz"
...
def set_step():  # ボタン押下でset_stepフラグをオン
    mrd.flag_set_step = 1
...
def callback(JointState):
    for i in range(11):
        ...
        mrd.s_meridim_js_sub[51+i *
                             2] = round(math.degrees(JointState.position[11+i])*100)*mrd.jspn[15+i]
...
def main():
    ...
    while (True):
        ...
        with dpg.window(label="Mini Terminal", width=248, height=175, pos=[600, 165]):
            # with dpg.group(label='LeftSide'):
            dpg.add_text("Index", pos=[15, 25])
            dpg.add_text("Data", pos=[60, 25])
            dpg.add_input_text(tag="s_index0", decimal=True, default_value="0", width=40, pos=[
                               15, 45])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data0", decimal=True, default_value=str(
                MSG_SIZE), width=60, pos=[60, 45])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_index1", decimal=True, default_value="", width=40, pos=[
                               15, 70])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data1", decimal=True, default_value="", width=60, pos=[
                               60, 70])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_index2", decimal=True, default_value="", width=40, pos=[
                               15, 95])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data2", decimal=True, default_value="", width=60, pos=[
                               60, 95])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_index3", decimal=True, default_value="", width=40, pos=[
                               15, 120])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data3", decimal=True, default_value="", width=60, pos=[
                               60, 120])  # 入力できるテキストボックスを設置

            dpg.add_text("Index", pos=[130, 25])
            dpg.add_text("Data", pos=[175, 25])
            dpg.add_input_text(tag="s_index4", decimal=True, default_value="", width=40, pos=[
                               130, 45])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data4", decimal=True, default_value="", width=60, pos=[
                               175, 45])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_index5", decimal=True, default_value="", width=40, pos=[
                               130, 70])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data5", decimal=True, default_value="", width=60, pos=[
                               175, 70])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_index6", decimal=True, default_value="", width=40, pos=[
                               130, 95])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data6", decimal=True, default_value="", width=60, pos=[
                               175, 95])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_index7", decimal=True, default_value="", width=40, pos=[
                               130, 120])  # 入力できるテキストボックスを設置
            dpg.add_input_text(tag="s_data7", decimal=True, default_value="", width=60, pos=[
                               175, 120])  # 入力できるテキストボックスを設置
            ...
            dpg.add_button(label="Flow",  callback=set_flow, pos=[
                           50, 148])  # Sendと書いてあるボタンをwindowの右下に設置
            dpg.add_button(label="Step",  callback=set_step, pos=[
                           90, 148])  # Sendと書いてあるボタンをwindowの右下に設置
        ...
        while dpg.is_dearpygui_running():
            ...
            # メッセージ欄の表示更新
            dpg.set_value("DispMessage0", mrd.message0)  # メッセージ欄表示用
            dpg.set_value("DispMessage1", mrd.message1)  # メッセージ欄表示用
            dpg.set_value("DispMessage2", mrd.message2)  # メッセージ欄表示用
            dpg.set_value("DispMessage3", mrd.message3)  # メッセージ欄表示用
            dpg.set_value("DispMessage4", mrd.message4)  # メッセージ欄表示用
            ...
            for i in range(0, 15, 1):
                ...
                dpg.set_value("ID L"+str(i), _idld/100)  # サーボIDと数値の表示L側
                dpg.set_value("ID R"+str(i), _idrd/100)  # サーボIDと数値の表示R側
            ...
            dpg.set_value("pad_Lx", mrd.r_meridim_char[33])
            dpg.set_value("pad_Ly", mrd.r_meridim_char[32])
            dpg.set_value("pad_Rx", mrd.r_meridim_char[35])
            dpg.set_value("pad_Ry", mrd.r_meridim_char[34])
            ...
            if (mrd.r_meridim[15] & 512 == 0):
                _padR2val = 0
            dpg.set_value("pad_L2v", _padL2val)
            dpg.set_value("pad_R2v", _padR2val)
            dpg.set_value("button_data", mrd.r_meridim[15])
